...
/* ===== GLOBAL STATE ===== */
const DOM_CACHE = {};
let weekdayArr = ['日曜日','月曜日','火曜日','水曜日','木曜日','金曜日','土曜日'];
...
function cacheDOM() {
    [
        ...
        'loading-container','app-container'
    ].forEach(id => {
...
function displayTime() {
    DOM_CACHE['current-time'].textContent = `${currentHour}:${pad2(currentMinute)}`;
...
function updateTomorrowMaterialsSummary() {
    ...
    updateMaterialsSummary('materials-tomorrow-section', 'tomorrows-materials-summary-list', t);
}
